...
<div class="card">
  ...
  <form method="POST" action="<?= base_url('expense/edit/'.$expense->id) ?>" class="space-y-5">
    ...
    <div>
      ...
      <div class="space-y-2">
        <div v-for="(item, i) in items" :key="i" class="flex gap-2 items-center">
          ...
          <input v-model.number="item.price" :name="'item_price[]'" type="number"
                 step="0.01" min="0" class="inp" style="width:110px" placeholder="ราคา"/>
...
createApp({
  setup() {
    const items = ref(initialItems.length ? initialItems : [{ name:'', price:0, qty:1 }])
